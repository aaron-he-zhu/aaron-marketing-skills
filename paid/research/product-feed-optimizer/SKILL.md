...
- **Writes**: a user-facing feed remediation package and reusable summary to `memory/paid-ads/product-feed-optimizer/`.
...
On user confirmation, save to `memory/paid-ads/product-feed-optimizer/YYYY-MM-DD-<catalog-or-goal>-feed.md` — see [Skill Contract](../../../references/skill-contract.md) §Save Results Template. Store the triage table, the title/attribute conventions, and the hygiene rules; do not store the full raw catalog.
