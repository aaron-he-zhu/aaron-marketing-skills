...
**Expected output**: a per-ad-set fatigue read — frequency now vs baseline, CTR and CVR decay slope against the early-flight baseline, the diagnosis (creative fatigue vs audience saturation vs neither), and one trigger (**Rotate-creative** / **Widen-audience** / **Hold**) with the threshold that fired — plus a handoff summary storable under `memory/paid-ads/fatigue-frequency-manager/`.
...
- **Writes**: a user-facing fatigue table plus a reusable summary storable under `memory/paid-ads/fatigue-frequency-manager/`.
...
Ask "Save these results?" If yes, write to `memory/paid-ads/fatigue-frequency-manager/` using `YYYY-MM-DD-<ad-set>-fatigue.md` — see [Skill Contract](../../../references/skill-contract.md) §Save Results Template. This skill asks before writing memory and hands off veto-like measurement risks to `ad-account-auditor` rather than marking a veto itself.
