...
Use this checklist before handing off Data Flow Skill work.
...
Use `DONE` when all deliverables are complete, `DONE_WITH_CONCERNS` when outputs are usable with caveats, `NEEDS_INPUT` when required user context is missing, and `BLOCKED` when analysis cannot proceed with available data.