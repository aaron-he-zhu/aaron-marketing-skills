...
description: 'Use when the user asks to "monitor brand mentions", "set up social listening", "did anything spike about us this week", or "watch these accounts for buying triggers". It runs always-on listening across keyless public sources and optional metered X search. It uses a versioned query architecture, a six-class mention sweep, a 7-day rolling baseline, B2B trigger watchlists, and explicit coverage labels. Xquik results are Measured for X; proxy sources never are. Not for launch-window telemetry; use launch-monitor. 舆情监听/品牌提及监测/基线突刺/触发信号'
version: "20.1.0"
license: Apache-2.0
...
- **Reads**: brand terms, product names, and exec handles (User-provided); the active-handle list from `memory/channels/` dossiers; prior baselines and query sets from `memory/social/social-pulse-monitor/`; keyless telemetry via `scripts/connectors/`: `bluesky.py`, `fediverse.py`, `discourse.py`, `hn.py`, `gdelt.py` (≥5s between calls), `tavily.py`, and `pageviews.py`; optional metered X search through `xquik.py`; other closed-platform mention exports (User-provided, as-of date).
- **Writes**: the pulse report to `memory/social/social-pulse-monitor/`; dated mention/activity lines submitted as individual idempotent proposal events to `memory/events/channels.ndjson` via an authorized `operation: propose` request to `registry-events.py` intra-day ([channel-registry](../../../protocol/channel-registry/SKILL.md) resolves each proposal in offset order).
...
Keyless Tier 1 remains `hn.py`, `bluesky.py`, `fediverse.py`, `discourse.py`, `gdelt.py`, `tavily.py`, and `pageviews.py`. X has an optional metered path: `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/connectors/xquik.py" listen "<brand>" "<rival>" --limit 10`. It returns bounded public X posts with query membership and pagination. Label those rows Measured for X at the connector's `as_of` time. Label GDELT, Tavily, or adjacent-network stand-ins proxy. Instagram, TikTok, LinkedIn, 小红书, 微信公众号, 视频号, and 抖音 still require user exports or proxy labels. See [CONNECTORS.md](../../../CONNECTORS.md).
...
1. **Build or refresh the listening-query architecture.** Brand-term variants: official name, common misspellings, 中文 names and transliterations, product names, exec handles. Exclusion terms: homonyms, unrelated same-name brands, own-handle noise. Per-source syntax: HN Algolia via `hn.py`, Bluesky via `bluesky.py`, GDELT via `gdelt.py`, Tavily via `tavily.py`, and X through `xquik.py listen`. Version the query set with a review date.
2. **Sweep the available surfaces.** Run the connectors against the same query set and dedupe cross-source hits. Label each count Measured for its own surface. Xquik rows are Measured for X. GDELT, Tavily, or Bluesky used as closed-platform stand-ins remain proxy.
...
6. **Write the coverage disclosure.** Name every swept surface and access class. X is Measured only when `xquik.py` supplied the row. Without that connector, X remains user-export or proxy. Instagram, TikTok, LinkedIn, 小红书, 微信公众号, 视频号, and 抖音 remain user-export or proxy. Never mark a proxy as Measured.
7. **Submit registry-grade lines.** Send each dated mention/activity/permission-lead line as an idempotent authorized `operation: propose` request through `registry-events.py` to `memory/events/channels.ndjson`; never edit the stream or projected views directly — [channel-registry](../../../protocol/channel-registry/SKILL.md) resolves proposals in offset order.
