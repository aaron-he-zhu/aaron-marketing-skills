...
This plugin provides **120 skills and 8 commands** across Narrative, SEO/GEO, Organic Social, Email, Paid Ads, Influencer, and Product Launch, plus a shared protocol layer. All 120 skills follow one contract and are auto-loaded by context; commands use `/aaron-marketing:`. The typed topology is [`references/system-catalog.json`](references/system-catalog.json); [`docs/system-architecture.md`](docs/system-architecture.md) is its generated human view.

<!-- GENERATED:BEGIN release-surface:current-bundle -->
Current bundle version: `18.0.0` (see [VERSIONS.md](https://github.com/aaron-he-zhu/aaron-marketing-skills/blob/main/VERSIONS.md)).
...
- Hook automation: `hooks/hooks.json` — seven command-backed events: SessionStart combined-budget memory/integrity context plus opt-in active-run resume, UserPromptSubmit, PreToolUse exact-path privacy preflight, PostToolUse and PostToolUseFailure post-state checks, PostToolBatch bounded namespace/sink sweep, and a first-Stop sweep with the required active-stop loop guard. Run lifecycle emission requires `AARON_ACTIVE_RUN_ID` plus stable host IDs and stores only hashed identity/typed metadata.
...
- Design philosophy: skills are content (Markdown). Core plugin/distribution runtime and repository-automation code under `scripts/` is **zero-dependency Python-stdlib/bash**, in three classes: (1) validators & generators (`validate-skill.sh`, `check-*.py`/`.sh`, `generate-*.py`), (2) deterministic runtimes invoked per `references/runtime-invocation.md` (`rubric-score.py`, `context-resolver.py`, `registry-events.py`, `run-events.py`, `audit-loop.py`, `audit-trends.py`, `validate-audit-artifact.py`), (3) connector helpers under `scripts/connectors/` that pull public/own data locally so skills don't need external tools (see [CONNECTORS.md](https://github.com/aaron-he-zhu/aaron-marketing-skills/blob/main/CONNECTORS.md)). No third-party / `pip` dependencies are permitted on those shipped or guard-scanned Python surfaces. Isolated probes under `evals/` are non-distributed and may pin their own path-scoped development toolchain.
