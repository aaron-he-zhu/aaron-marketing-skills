...
into HTML, Markdown, and JSON. It is **not** a 121st Skill, **not** an OSS
install surface, **not** a hosted Web product, and **not** a Gateway.
...
- Kept out of the plugin / Portable Lite / governed package (560-file
  ceiling). `scripts/skill-dashboard.py`, this guide, and
  `references/skill-dashboard.schema.json` are maintenance paths.
...
```bash
python3 scripts/skill-dashboard.py --root /path/to/project
