...
- [Install](#install)
  - [AI Staff — the marketing department, hired as bots](#ai-staff--the-marketing-department-hired-as-bots)
- [First run](#first-run)
...
| **Agent Plugins v1 clients · Portable Lite** | Download `aaron-marketing-skills-20.0.0-agent-plugin-v1-lite.tar.gz` from the [v20.0.0 release](https://github.com/aaron-he-zhu/aaron-marketing-skills/releases/tag/v20.0.0), unpack it, and install the extracted plugin directory |
| **Grok Bot · Hermes Bot Mode (AI Staff)** | Generate the 8-bot roster: `python3 scripts/generate-bot-projections.py --output <private-dir>` — 7 specialists + `aaron-chief`. See [AI Staff](#ai-staff--the-marketing-department-hired-as-bots) |
...
### AI Staff — the marketing department, hired as bots

On named-bot hosts (xAI's **Grok Bot**, Hermes Agent's **Bot Mode**) this bundle doesn't install as a pile of 120 skills — it installs as **staff**: eight named teammates you message like colleagues. Seven specialists each own one lane — `aaron-narrative`, `aaron-seo-geo`, `aaron-social`, `aaron-email`, `aaron-ad`, `aaron-influencer`, `aaron-launch` — and **`aaron-chief`** runs the desk: it holds the 8 protocol registries and routes any cross-lane goal by @mention (visited set, three-handoff cap). It is the same chief-of-staff-plus-specialists org chart these hosts are built around, derived from the same typed catalog as every other install form: all 120 skills, covered exactly once, no second inventory.

Tell `@aaron-chief` "launch v2 on Product Hunt in three weeks" and the plan comes back through `@aaron-launch`, `@aaron-email`, and `@aaron-social` — each bot answers in its own lane; none of them freelances outside it.

```bash
python3 scripts/generate-bot-projections.py --output /private/path/aaron-bot-roster
```

The generator writes 8 installable Hermes profile bundles (`hermes/<bot>/`, hash-bound manifests) plus the Grok Bot setup pack (`grok/bot-cards.md`, per-bot enable lists, setup checklist). Staff bundles are **Tier-1 static**: no connectors, MCP, cron, or runtimes; auditors return `NOT_SCORED` rather than guessing; registry and memory writes stay propose-only. On Grok Bot, all of a member's bots share one cloud computer — bot names are not security boundaries. Deployment steps, host caveats, and the owner-run smoke backlog: [agent-compatibility.md](docs/agent-compatibility.md#named-bot-roster-deployment-grok-bot--hermes-bot-mode).
...
| ⛩ social-quality-auditor | Asset gate / program maturity | Auditor-class ECHO gate: scores one declared unit/profile, enforces applicable E1/C1/C2/H1/H2/O1 controls, and emits SHIP/FIX/BLOCK. |
...
| ⛩ launch-readiness-auditor | Preflight / execution / outcome | Auditor-class RAMP gate: scores one lifecycle profile, enforces profile-relevant R1/A1/M1/P1 controls, and emits SHIP/FIX/BLOCK. |
...
| Skill | What it does |
|-------|--------------|
| entity-registry | Canonical entity profile for Knowledge Graph, Wikidata, AI disambiguation. |
...
| **Vendor MCP servers** | 20 opt-in catalog entries — 19 vendor-hosted remote endpoints plus one self-hosted OpenSEO entry — are never auto-registered, so your `/mcp` list stays clean. The official self-hosted servers for Google Analytics, Search Console, **Google Ads**, and **Microsoft Clarity** are also documented. Two remote MCPs work with no key at all (Firecrawl, Tavily). |
...
- **Opt-in MCP servers** (Ahrefs, Semrush, SE Ranking, SISTRIX, SimilarWeb, the self-hosted free **OpenSEO** suite, Cloudflare, Vercel, HubSpot, Amplitude, Notion, Webflow, Sanity, Contentful, Slack, Resend, the keyless Firecrawl and Tavily, Appeeky, and Upfluence) are catalogued in [`docs/mcp-catalog.json`](docs/mcp-catalog.json) as a **copy-paste reference only** — the catalog sits outside the auto-registered plugin-root `.mcp.json` path, so nothing is registered for you. Copy the entries you want into your own MCP config.
...
| `tests/test_context_assembly.py` + `tests/test_context_profile_resolver.py` | Controller/model/tool separation, host capability resolution, prompt-profile certification gates, capsule/kernel substitution, and evaluation-only non-deployability. |
| `tests/test_prompt_profile_evidence.py` + `tests/test_codex_behavior_adapter.py` | Protocol-v3 blind routing, selected-source isolation, paired-evidence arithmetic/safety gates, nullable provider usage, and compact-profile non-deployability. |
...
| `generate-skill-capsules.py --check` + `generate-claude-index.py --check` | Proves compact model capsules and the bounded root discovery index remain exact generated views of live Skills/catalogs. |
| `workflow-graph.py --check` + `tests/test_workflow_graph.py` | Proves the authoritative 120-node/378-edge graph, typed conditions, bounded re-entry, reachability, and Product Launch fan-out/join projection. |
...
| `check-context-budget.py` + `check-context-efficiency.py` | Root-navigation, capsule, recursive-reference, real-manifest, and largest valid three-shard `/auto` context ceilings. |
