...
- **Reads**: the brand and locked competitor panel; the listening-query architecture from `memory/social/social-pulse-monitor/`; the own-handle list from `memory/channels/`; keyless counts via `hn.py`, `bluesky.py`, `fediverse.py`, `discourse.py`, `gdelt.py`, `tavily.py`, and `pageviews.py`; optional metered X counts from `xquik.py listen`; other closed-platform counts from dated user exports.
- **Writes**: the SOV read plus the panel record with its lock date and break log to `memory/social/share-of-voice-tracker/`.
- **Promotes**: confirmed SOV shifts (panel and denominator named) and any panel break to `memory/hot-cache.md` / `memory/open-loops.md` (ask before writing); competitor-strategy observations route to [competitor-tracker](../../../influencer/target/competitor-tracker/SKILL.md) instead of being stored here; any channel fact surfaced goes to `memory/events/channels.ndjson` via an authorized `operation: propose` request to `registry-events.py` only.
...
Use the same keyless surfaces as [social-pulse-monitor](../social-pulse-monitor/SKILL.md). For X, run one locked panel through `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/connectors/xquik.py" listen "<brand>" "<competitor>" --limit 10`. The helper merges duplicates but retains every matching query. Count each X post once per panel member it matches, using one declared overlap rule. Keep the same page size, sort, date window, and filters across the panel. Xquik counts are Measured for X at the connector's `as_of` time. GDELT and Tavily remain proxy. Other closed platforms require dated user exports. See [CONNECTORS.md](../../../CONNECTORS.md).
...
3. **Pull counts per brand, per platform, per period.** Use identical windows and syntax across the panel. For X, keep one `xquik.py listen` invocation per comparison and apply the locked overlap rule to `matched_queries`. Label Xquik, HN, Bluesky, fediverse, and Discourse counts Measured for their own surfaces. Label GDELT and Tavily proxy. Label modeled fills Estimated. Respect GDELT's ≥5s spacing.
...
- [competitor-tracker](../../../influencer/target/competitor-tracker/SKILL.md) — competitor content and strategy watching (not this skill)
- [offsite-signal-analyzer](../../../seo-geo/evaluate/offsite-signal-analyzer/SKILL.md) — backlink and offsite SEO signals (not this skill)
...
- **If the user asks what competitors are doing**: [competitor-tracker](../../../influencer/target/competitor-tracker/SKILL.md) — content and strategy watching lives there.
